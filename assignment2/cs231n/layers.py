...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        x_nor = (x - mean) / np.sqrt(var + eps)
        out = gamma * x_nor + beta

        running_mean = momentum * running_mean + (1 - momentum) * mean
        running_var = momentum * running_var + (1 - momentum) * var

        cache = (x_nor, x_mean, inv_var, gamma)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    N, D = dout.shape
    x_nor, x_mean, inv_var, gamma = cache
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(dout * x_nor, axis=0)
    dx_nor = dout * gamma

    dx = (1. / N) * inv_var * (N*dx_nor - np.sum(dx_nor, axis=0) \
            - x_nor*np.sum(dx_nor*x_nor, axis=0))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
        ...
        for ff in range(F):
            for hh in range(out_H):
                for ww in range(out_W):
                    out[nn, ff, hh, ww] = \
                            np.sum(x_padding[nn, :, hh*stride:hh*stride+HH, ww*stride:ww*stride+WW] * \
                                   w[ff, :, :, :]) + b[ff]
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_backward_naive(dout, cache):
    ...
    dw = np.zeros(w.shape)
    dx_pad = np.zeros(x_padding.shape)
    db = np.zeros(b.shape)
    ...
    for nn in range(N):
        for ff in range(F):
            for hh in range(out_H):
                for ww in range(out_W):
                    dw[ff, :, :, :] += dout[nn, ff, hh, ww] * \
                            x_padding[nn, :, hh*stride:hh*stride+HH, ww*stride:ww*stride+WW]

    for nn in range(N):
        for ff in range(F):
            for hh in range(out_H):
                for ww in range(out_W):
                    dx_pad[nn, :, hh*stride:hh*stride+HH, ww*stride:ww*stride+WW] += \
                            w[ff, :, :, :] * dout[nn, ff, hh, ww]
...
def max_pool_backward_naive(dout, cache):
    ...
    for nn in range(N):
        for cc in range(C):
            for hh in range(dout_H):
                for ww in range(dout_W):
                    max_index = np.argmax(x[nn, cc, hh*stride:hh*stride+pool_height, ww*stride:ww*stride+pool_width])
                    max_coord = np.unravel_index(max_index, [pool_height, pool_width])
                    dx[nn, cc, hh*stride:hh*stride+pool_height, ww*stride:ww*stride+pool_width][max_coord] = dout[nn,cc,hh,ww]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    x_reshape = x_T.reshape(-1, x.shape[1])

    out_reshape, cache = batchnorm_forward(x_reshape, gamma, beta, bn_param)
    out_T = out_reshape.reshape(*x_T.shape)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dout_reshape = dout_T.reshape(-1, dout.shape[1])

    dx_reshape, dgamma, dbeta = batchnorm_backward(dout_reshape, cache)

    dx_T = dx_reshape.reshape(*dout_T.shape)
